...
export function HomePage() {
  ...
  return (
    <>
      ...
      <div className="">
        {isLoading ? isPending && <div>Loading Riot Account...</div> : null}
      ...
      <div className="">
        ...
        {profileError && <div>Error: {(profileError as Error).message}</div>}
        {!!profile && (
          <div className="mt-4">
            <h2 className="mb-2 text-lg font-semibold">Summoner Profile</h2>
            <div className="space-y-2">
              <p>
                <strong>Summoner Level:</strong> {profile.summonerLevel}
              </p>
              <p>
                <strong>Profile Icon ID:</strong> {profile.profileIconId}
              </p>
              <p>
              ...
              <p>
                ...
                {new Date(profile.revisionDate).toLocaleString()}
              </p>
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <div className="space-y-4">
              {matches.map((match, index) => {
                if (match.isPending) return <div key={index}>Loading match...</div>;
                if (match.error)
                  return (
                    <div key={index}>
                      Error: {(match.error as Error).message}
                    </div>
                  );
                ...
                const gameDurationMinutes = Math.floor(info.gameDuration / 60);
                const gameDurationSeconds = info.gameDuration % 60;
                  ...
                  <div key={index} className="border rounded-lg p-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="font-semibold">{info.gameMode}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="text-sm">{gameDate}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Queue ID</p>
                        <p className="font-semibold">{info.queueId}</p>
                      </div>
                    </div>
                    <div className="mt-3 pt-3 border-t">
                      <p className="text-sm text-gray-500 mb-2">Teams</p>
                      <div className="grid grid-cols-2 gap-2">
                        {info.teams.map((team: typeof info.teams[0]) => (
                          <div
                            key={team.teamId}
